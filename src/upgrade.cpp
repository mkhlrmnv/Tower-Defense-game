...
void Upgrade::setup_menu(){
    ...
    auto light_grey = sf::Color(80,90,80);
    auto transparent_grey = sf::Color(80, 90, 80, 100);
    ...
    _background.setFillColor(transparent_grey);
    _background.setOutlineColor(light_grey);
    ...
    setup_text_line(_dmg_img, _dmg_text, { attrs_start_x, attrs_start_y + 2*(line_spacing + char_size)}, white, TowerAttributes::RNG, char_size ); // TODO: MAKE IMAGE for DMG
    ...
    _price_text.setPosition({attrs_start_x, attrs_start_y + 5*(line_spacing + char_size)});
    
    //_upgrade_button_img.setTexture(_rh.get_texture_attribute(TowerAttributes::UPGRADE));
    //sprite.setScale(scale, scale);
    //_upgrade_button_img.setPosition({attrs_start_x, attrs_start_y + 5*(line_spacing + char_size)});
    _upgrade_button= new Button("UPGRADE", {80, 30}, {attrs_start_x, attrs_start_y + 6*(line_spacing + char_size)}, sf::Color::Green, sf::Color::Black, _rh.get_font());
...
void Upgrade::set_menu(sf::Vector2f position){
    ...
    */
    ...
    int rng = _tower_to_upgrade->get_damage();
    int atkspd = _tower_to_upgrade->get_damage();
    ...
    _price_text.setPosition({attrs_start_x, attrs_start_y + 5*(line_spacing + char_size)});
    _upgrade_button->set_position({attrs_start_x, attrs_start_y + 6*(line_spacing + char_size)});
    _upgrade_button->center_text();
}   
